...
def parse_results(file_content):
    ...
    if mse_diff_means_section:
        mse_diff_means = re.findall(r'MSE Difference for (\w+ \d+): ([0-9.e+-]+)', mse_diff_means_section.group(1))
        parsed_data['mse_difference_means'] = {panel: float(mse) for panel, mse in mse_diff_means}
    ...
    if mse_diff_stds_section:
        mse_diff_stds = re.findall(r'MSE Difference for (\w+ \d+): ([0-9.e+-]+)', mse_diff_stds_section.group(1))
        parsed_data['mse_difference_stds'] = {panel: float(mse) for panel, mse in mse_diff_stds}

    # Extract 1D TVD for each feature
    tvd_1d_section = re.search(r'1D TVD for each feature:(.*?)\n\n', file_content, re.DOTALL)
    if tvd_1d_section:
        tvd_1d_panels = re.findall(r'1D TVD for (\w+ \d+):\n(\[[^\]]+\])', tvd_1d_section.group(1))
        mean_1d_tvd = re.findall(r'Mean 1D TVD for (\w+ \d+):\s*([0-9.e+-]+)', tvd_1d_section.group(1))
        parsed_data['1d_tvd'] = {panel: np.array(eval(fix_array_syntax(values))) for panel, values in tvd_1d_panels}
        parsed_data['mean_1d_tvd'] = {panel: float(mean) for panel, mean in mean_1d_tvd}
    ...
    if tvd_2d_section:
        ...
        mean_2d_tvd = re.findall(r'Mean 2D TVD for (\w+ \d+):\s*([0-9.e+-]+)', tvd_2d_section.group(1))
        ...
        parsed_data['mean_2d_tvd'] = {panel: float(mean) for panel, mean in mean_2d_tvd}

    # Extract Average Cluster Distance
    avg_cluster_dist_section = re.search(r'Average Cluster Distance:(.*?)(?:\n\n|$)', file_content, re.DOTALL)
    if avg_cluster_dist_section:
        avg_cluster_dist = re.findall(r'Average Cluster Distance for (\w+ \d+): ([0-9.e+-]+)', avg_cluster_dist_section.group(1))
        parsed_data['average_cluster_distance'] = {panel: float(dist) for panel, dist in avg_cluster_dist}
...
if __name__ == "__main__":
    ...
    path = f"{batch}/{method}/results_e1.txt"
