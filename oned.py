...
def plot_fluoro_hist_compare(data_list, names=None, idx=None, synth_batch=True, file_name=None):
    ...
    if num_datasets < 1:
        print("Error: At least one dataset are required for comparison.")
        return
...
if __name__ == "__main__":
    ...
    batches = ["Plate 27902_N", "Plate 28528_N"]
    data_list = []
    names = []
    for i, batch in enumerate(batches):
        ...
        if (i == 0):
            ...
            names.append(batch + " Corrected")

    raw_data_batches = batches[1:]
    for batch in raw_data_batches:
        filename = f"{dataset}/rawdata/{batch}.npy"
        data = np.load(filename)
        data_list.append(data)
        names.append(batch + " Uncorrected")


    plot_fluoro_hist_compare(data_list, idx=7, names=names, synth_batch=False, file_name=f"{directory}")